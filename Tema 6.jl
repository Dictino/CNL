...
ϕ1=-x1^2 - k1*x1  #el  valor deseado de x2
...
md"Y derivemos a ver que sale, para ello primero calculo $\dot z$"
...
function control(x,p,t)
	...
	dϕ1=-k1*x1^2 + k1*x1^3 - k1*x2 - 2*x2*x1 - 2*x1^3 + 2*x1^4
	
	ϕ2=dϕ1 - x1 - k2*z1
	
	z2=x3-ϕ2
	
	dϕ2=-x2 - k1*(x1^2 + x1^3 - x2)+ k2*(ϕ1 - x3)- 2*x2*x1 - x1^2 - x1^3 + 2*x1^4
...
expand(control([x1 x2 x3],[k1,k2,k3],0))
...
# ╔═╡ 23797110-5fdb-11eb-0035-6fb7c83e279c
k_1

# ╔═╡ fc5e8b30-4147-11eb-30d3-4bef0bac9ca6
let
	x0 = [1 0 0]
	parametros=[k_1 k_2 k_3] #cambiar
	tspan = (0.0,10.0)
